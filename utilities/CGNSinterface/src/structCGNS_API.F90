...
subroutine openFile(fileName, mode, cg)
  ...
  ! F2PY INTENTS
  !f2py intent(in) fileName, mode
  !f2py intent(out) cg

  ! Input/Output
  character*(*), intent(in) :: fileName
  integer(kind=intType), intent(in) :: mode
  integer(kind=intType), intent(out) :: cg

  ! Working 
  integer(kind=intType) :: ier, base
...
subroutine getGridDimension(cg, cellDim)
  ...
  ! F2PY INTENTS
  !f2py intent(in) cg
  !f2py intent(out) cellDim

  ! Input/Output
  integer(kind=intType), intent(in) :: cg
  integer(kind=intType), intent(out) :: cellDim

  ! Working
  integer(kind=intType) :: ier, base, physDim
...
subroutine getBlockInfo(cg, iBlock, zoneName, dims, nBoco, nB2B)
  ...
  ! F2PY INTENTS
  !f2py intent(in) cg, iBlock
  !f2py intent(out) zoneName, dims, nBoco, nB2B

  ! Input/Output
  integer(kind=intType), intent(in) :: cg, iBlock
  integer(kind=intType), intent(out) :: dims(3)
  character(len=512), intent(out) :: zoneName
  integer(kind=intType), intent(out) :: nBoco, nB2B
  ! Working
  integer(kind=intType) :: ier, base, tmp(9)
...
subroutine getCoordinates3D(cg, iBlock, il, jl, kl, X)
  ...
  ! F2PY INTENTS
  !f2py intent(in) cg, iBlock, il, jl, kl
  !f2py intent(out) X

  ! Input/Output
  integer(kind=intType), intent(in) :: cg, iBlock, il, jl, kl
  real(kind=realType), intent(out) :: X(il, jl, kl, 3)

  ! Working
  integer(kind=intType) :: ier, base, blockStart(3), blockEnd(3)
...
end subroutine getCoordinates3D

subroutine getCoordinates2D(cg, iBlock, il, jl, X)
  ! Return the subset of block coordinates of block 'iBlock', of dimensions
  ! (il, jl, 3) in X
  ...
  ! F2PY INTENTS
  !f2py intent(in) cg, iBlock, il, jl
  !f2py intent(out) X

  ! Input/Output
  integer(kind=intType), intent(in) :: cg, iBlock, il, jl
  real(kind=realType), intent(out) :: X(il, jl, 3)

  ! Working
  integer(kind=intType) :: ier, base, blockStart(2), blockEnd(2)
...
end subroutine getCoordinates2D
