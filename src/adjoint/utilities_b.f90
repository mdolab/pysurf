...
! Rescale newArcLength based on the final distance
    CALL PUSHREAL8ARRAY(newarclength, nnewnodes)
...
&             newcoor(1, :), newcoorb(1, :))
    CALL POPREAL8ARRAY(newarclength, nnewnodes)
...
&                      newarclength, newarclengthb)
        arclengthb(nelem+1) = arclengthb(nelem+1) - sp2*arg2b/arclength(&
&         nelem+1)**2 - sp1*arg10b/arclength(nelem+1)**2
      END IF
    END IF
    coorb = 0.0_8
    DO elemid=nelem,1,-1
...
!
    ...
    DO interp=interp_num,1,-1
      ...
      t_datab(right) = t_datab(right) + p_data(left)*tempb
      tb = (p_data(right)-p_data(left))*tempb
      p_datab(left) = p_datab(left) + (t_data(right)-t)*tempb
      t_datab(left) = t_datab(left) - p_data(right)*tempb
...
&       (right))*tempb/(t_data(right)-t_data(left)))
      t_datab(right) = t_datab(right) + tempb0
      t_datab(left) = t_datab(left) - tempb0
...
! Compute parameter A
 110 a = SQRT(sp1/sp2)
    ...
    DO i=n,1,-1
      temp = 2*a + (-a+1)*u
      tempb = -(u*spacingsb(i)/temp**2)
      ub = spacingsb(i)/temp + (1-a)*tempb
      spacingsb(i) = 0.0_8
      ab = ab + (2-u)*tempb
      CALL POPREAL8(u)
      temp = TANH(b/2)
      bb = bb + (r*(1.0-TANH(r*b)**2)/temp-(1.0-TANH(b/2)**2)*TANH(r*b)/&
&       (2*temp**2))*ub
      CALL POPREAL8(r)
    END DO
...
!   with respect to varying inputs: sp1 sp2 b
  SUBROUTINE FINDROOTB_B(b, bb, sp1, sp1b, sp2, sp2b, n, b_out, b_outb)
    ...
    bb = bb + COSH(b)*b_outb + tempb
    IF (sp1*sp2 .EQ. 0.0) THEN
      tempb0 = 0.0_8
    ELSE
      tempb0 = -(b*tempb/(2.0*temp**2))
    END IF
