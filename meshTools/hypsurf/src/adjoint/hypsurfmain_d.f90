...
CONTAINS
!  Differentiation of computematrices_main in forward (tangent) mode:
...
&   epse0, guideindices, retainspacing, f, fd, numnodes, numguides)
    USE SOLVEROUTINES_D, ONLY : solve, solve_d
    IMPLICIT NONE
...
&                    , theta, numnodes, k, kd, f, fd)
    ...
    DO index=2,numnodes-1
      ...
      END IF
      IF (guide) THEN
        kd(3*(index-1)+1, 3*(index-1)+1) = 0.0
        k(3*(index-1)+1, 3*(index-1)+1) = one
        kd(3*(index-1)+2, 3*(index-1)+2) = 0.0
        k(3*(index-1)+2, 3*(index-1)+2) = one
        kd(3*(index-1)+3, 3*(index-1)+3) = 0.0
...
! Populate matrix
      kd(3*index-0, 3*(index-2)+1) = 0.0
      k(3*index-0, 3*(index-2)+1) = zero
      kd(3*index-0, 3*(index-2)+2) = 0.0
      k(3*index-0, 3*(index-2)+2) = zero
      kd(3*index-0, 3*(index-2)+3) = 0.0
      k(3*index-0, 3*(index-2)+3) = -one
      kd(3*index-1, 3*(index-2)+1) = 0.0
      k(3*index-1, 3*(index-2)+1) = zero
      kd(3*index-1, 3*(index-2)+2) = 0.0
      k(3*index-1, 3*(index-2)+2) = -one
      kd(3*index-1, 3*(index-2)+3) = 0.0
...
! Populate matrix
      kd(3*index-2, 3*(index-2)+1) = 0.0
      k(3*index-2, 3*(index-2)+1) = -one
      kd(3*index-2, 3*(index-2)+2) = 0.0
      k(3*index-2, 3*(index-2)+2) = zero
      kd(3*index-2, 3*(index-2)+3) = 0.0
...
!  Differentiation of matrixbuilder in forward (tangent) mode:
...
!  Differentiation of dissipationcoefficients in forward (tangent) mode:
...
! Compute Sl (Eq. 6.5) based on a transition l of 3/4 of max
    ...
    ELSE
      pwy1 = 2./sl
      IF (dsensor .GT. 0.0 .OR. (dsensor .LT. 0.0 .AND. pwy1 .EQ. INT(&
&         pwy1))) THEN
        dbard = pwy1*dsensor**(pwy1-1)*dsensord
      ELSE IF (dsensor .EQ. 0.0 .AND. pwy1 .EQ. 1.0) THEN
        dbard = dsensord
      ELSE
        dbard = 0.0
      END IF
...
!  Differentiation of areafactor in forward (tangent) mode:
...
!  Differentiation of smoothing_main in forward (tangent) mode:
...
!  Differentiation of findradius in forward (tangent) mode:
...
!  Differentiation of findratio in forward (tangent) mode:
...
! Residual function
      pwy1 = numlayers - 1
      IF (q .GT. 0.0 .OR. (q .LT. 0.0 .AND. pwy1 .EQ. INT(pwy1))) THEN
        pwr1d = pwy1*q**(pwy1-1)*qd
      ELSE IF (q .EQ. 0.0 .AND. pwy1 .EQ. 1.0) THEN
        pwr1d = qd
      ELSE
        pwr1d = 0.0
      END IF
...
!  Differentiation of redistribute_nodes_by_arc_length in forward (tangent) mode:
...
!  Differentiation of compute_arc_length in forward (tangent) mode:
...
!  Differentiation of matinv3 in forward (tangent) mode:
...
!  Differentiation of giveangle in forward (tangent) mode:
