...
def march(
    ...
    if max(abs(rNext - rStart)) > 1.0e-5:
        warn(
            "The given points (rStart) might not belong to the given surface (surf)\nThese points were projected for the surface mesh generation"
        )
    ...
    for layerIndex in range(numLayers - 1):
        ...
        for indexSubIter in range(cFactor):
            ...
            rNext, NNext = subIteration(
                r0, N0, S0, rm1, Sm1, layerIndex, guideIndices, arcLength, extraData, projection_func
            )
...
def areaFactor(r0, d, nuArea, numAreaPasses, bc1, bc2, guideIndices):
    ...
    neighborDist = 0.5 * (
        np.linalg.norm(R0_extrap[:, 1:-1] - R0_extrap[:, :-2], axis=0)
        + np.linalg.norm(R0_extrap[:, 2:] - R0_extrap[:, 1:-1], axis=0)
    )
...
def computeMatrices(r0, N0, S0, rm1, Sm1, layerIndex, guideIndices, extraData):
    ...
    def matrixBuilder(curr_index):
        ...
        if curr_index == 0:  # forward case
            ...
            if bc1 != "continuous":
                ...
                r0_xi = 0.5 * (
                    -3 * r0[3 * (curr_index) : 3 * (curr_index) + 3]
                    + 4 * r0[3 * (neighbor1_index) : 3 * (neighbor1_index) + 3]
                ...
                r0_xi = 0.5 * (
                    r0[3 * (neighbor2_index) : 3 * (neighbor2_index) + 3]
                    - r0[3 * (neighbor1_index) : 3 * (neighbor1_index) + 3]
                )
        ...
        dnum = np.linalg.norm(
            rm1[3 * (neighbor2_index) : 3 * (neighbor2_index) + 3] - rm1[3 * (curr_index) : 3 * (curr_index) + 3]
        ) + np.linalg.norm(
            rm1[3 * (neighbor1_index) : 3 * (neighbor1_index) + 3] - rm1[3 * (curr_index) : 3 * (curr_index) + 3]
        )
        dden = np.linalg.norm(
            r0[3 * (neighbor2_index) : 3 * (neighbor2_index) + 3] - r0[3 * (curr_index) : 3 * (curr_index) + 3]
        ) + np.linalg.norm(
            r0[3 * (neighbor1_index) : 3 * (neighbor1_index) + 3] - r0[3 * (curr_index) : 3 * (curr_index) + 3]
        )
        dSensor = dnum / dden
        ...
        if angle < 70 * np.pi / 180:  # Corner detected
            ...
            if curr_index == 0:
                ...
                if bc1 != "continuous":  # forwards
                    De = epsE * (
                        r0[3 * (curr_index) : 3 * (curr_index) + 3]
                        - 2 * r0[3 * (neighbor1_index) : 3 * (neighbor1_index) + 3]
                        + r0[3 * (neighbor2_index) : 3 * (neighbor2_index) + 3]
                    )
                    ...
                    # Populate matrix
                    K[
                        3 * (curr_index) : 3 * (curr_index) + 3, 3 * (neighbor2_index) : 3 * (neighbor2_index) + 3
                    ] = L_block
                    K[
                        3 * (curr_index) : 3 * (curr_index) + 3, 3 * (neighbor1_index) : 3 * (neighbor1_index) + 3
                    ] = M_block
                    ...
                    De = epsE * (
                        r0[3 * (neighbor1_index) : 3 * (neighbor1_index) + 3]
                        - 2 * r0[3 * (curr_index) : 3 * (curr_index) + 3]
                        + r0[3 * (neighbor2_index) : 3 * (neighbor2_index) + 3]
                    )
    ...
    for index in [0]:
        ...
        if bc1 == "splay":
            ...
            # Populate matrix
            K[3 * index : 3 * index + 3, 3 * index : 3 * index + 3] = np.array(
                [
                    [d_vec_rot[0], d_vec_rot[1], d_vec_rot[2]],
...
def smoothing(r, eta, alphaP0, numSmoothingPasses, numLayers, numNodes):
    ...
    for index_pass in range(numSmoothingPasses):
        ...
        for index in range(1, numNodes - 1):
            ...
            r_smooth[3 * index : 3 * index + 3] = (1 - alphaP) * r_curr + alphaP * (lm * r_next + lp * r_prev) / (
                lp + lm
            )
...
def qualityCheck(R, layerIndex=None):
    ...
    nodalDerivs[:, 0, 1:-1, 1:-1] = (XYZ[:, 2:, 1:-1] - XYZ[:, :-2, 1:-1]) / 2.0
    nodalDerivs[:, 1, 1:-1, 1:-1] = (XYZ[:, 1:-1, 2:] - XYZ[:, 1:-1, :-2]) / 2.0

    # Compute i derivatives using 1st order differencing
    nodalDerivs[:, 0, 0, :] = XYZ[:, 1, :] - XYZ[:, 0, :]
    nodalDerivs[:, 0, -1, :] = XYZ[:, -1, :] - XYZ[:, -2, :]

    nodalDerivs[:, 0, 1:-1, 0] = (XYZ[:, 2:, 0] - XYZ[:, :-2, 0]) / 2.0
    nodalDerivs[:, 0, 1:-1, -1] = (XYZ[:, 2:, -1] - XYZ[:, :-2, -1]) / 2.0

    # Compute j derivatives using 1st order differencing
    nodalDerivs[:, 1, :, 0] = XYZ[:, :, 1] - XYZ[:, :, 0]
    nodalDerivs[:, 1, :, -1] = XYZ[:, :, -1] - XYZ[:, :, -2]

    nodalDerivs[:, 1, 0, 1:-1] = (XYZ[:, 0, 2:] - XYZ[:, 0, :-2]) / 2.0
    nodalDerivs[:, 1, -1, 1:-1] = (XYZ[:, -1, 2:] - XYZ[:, -1, :-2]) / 2.0
