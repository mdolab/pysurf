...
class SurfaceMesh(object):
        ...
        if isinstance(arg, dict):
            ...
            if extension == ".cgns":
                ...
                for blockID in range(grid.num_blocks):
                    ...
                    blockCoor = np.zeros([grid.blocks[blockID].X.shape[0], grid.blocks[blockID].X.shape[1], 3])
    ...
    def set_points(self, flatArray):
        ...
        for block in self.coor:
            ...
            self.coor[block][:, :, :] = flatArray[offset : offset + numPoints, :].reshape(
                (dims[0], dims[1], 3), order="F"
            )
    ...
    def accumulate_reverseADSeeds(self, flatArray):
        ...
        for block in self.coor:
            ...
            self.coorb[block] = self.coorb[block] + flatArray[offset : offset + numPoints, :].reshape(
                (dims[0], dims[1], 3), order="F"
            )
    ...
    def set_randomADSeeds(self, mode="both", fixedSeed=True):
        ...
        if mode == "forward" or mode == "both":
            ...
            for block in self.coor:
                ...
                coord = np.array(
                    np.random.rand(self.coor[block].shape[0], self.coor[block].shape[1], self.coor[block].shape[2])
                )
    ...
    def export_plot3d(self, fileName):
        ...
        for block in self.coor:
            myGrid.add_block(self.coor[block][:, :, 0:1], self.coor[block][:, :, 1:2], self.coor[block][:, :, 2:3])
