...
class tfi(object):
    ...
    def __init__(
        ...
        ----------
        nu : integer
            number of nodes in u direction (including end points)
        nv : integer
            number of nodes in v direction (including end points)
        alpha00 : function alpha00(u)
            univariate function used to interpolate functions in u
            alpha00(0) = 1.0 and alpha00(1) = 0.0
        alpha01 : function alpha01(u)
            univariate function used to interpolate derivatives in u
        alpha10: function alpha10(u)
            univariate function used to interpolate functions in u
            alpha10(0) = 0.0 and alpha10(1) = 1.0
        alpha11: function alpha11(u)
            univariate function used to interpolate derivatives in u
        beta00:  function beta00(v)
            univariate function used to interpolate functions in v
            beta0(0) = 1.0 and beta0(1) = 0.0
        beta01:  function beta01(v)
            univariate function used to interpolate derivatives in v
        beta10:  function beta10(v)
            univariate function used to interpolate functions in v
            beta1(0) = 0.0 and beta1(1) = 1.0
        beta11:  function beta11(v)
            univariate function used to interpolate derivatives in v
    ...
    def setTFI(
        ...
        u: array[nu]
            Parametric coordinates (ranging from 0 to 1) that correspond to each u-layer.

        v: array[nv]
            Parametric coordinates (ranging from 0 to 1) that correspond to each v-layer.
    ...
    def interpolate(self, F, dFdu, dFdv, ddFdudv):
        ...
        ----------
        F: array[nu,nv]
            This array should have the boundary values for the interpolation.
            The values in the interior of the array will not be used.
            They will be replaced by the interpolated values::
        ...
        F: array[nu,nv]
            array containing the values f(u,v). Only the boundary
            values will be used in this function. We will assume
            that the values in f are in homogeneous parametric spacing

        dFdu: array[2,nv]
            Array with dFdu for nodes on the top [0,:]
            and bottom nodes [1,:]

        dFdv: array[nu,2]
            Array with dFdv for nodes on the left [:,0] and right nodes [:,1]

        ddFdudv: array[2,2]
            Array with ddFdudv for the corner nodes:
        ...
        F: array[nu,nv]
            Updated array with interpolated values in the interior elements::
        ...
        Warnings
        --------
        F cannot be an array of integers because when we assign float to
    ...
    def __computeReferenceArrays(self, F, dFdu, dFdv, ddFdudv):
        ...
        ----------
        F: array[nu,nv]
            array containing the values f(u,v).
            Only the boundary values will be used in this function.
            We will assume that the values in f are in homogeneous parametric spacing

        dFdu: array[2,nv]
            Array with dFdu for nodes on the top [0,:] and bottom nodes [1,:]
