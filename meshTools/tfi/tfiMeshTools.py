...
def computeMesh(X, Y, Z, derivDict=None):
    ...
    tfi = tfiLib.tfi(nu, nv, alpha00, alpha01, alpha10, alpha11, beta00, beta01, beta10, beta11)
...
def computeDerivatives(X, Y, Z, s0u, s0v, N0u, N0v, blendingFactor=0.6):
    ...
    def computeDerivativesOnLine(
        Xl, Yl, Zl, Xm0, Ym0, Zm0, Xm1, Ym1, Zm1, nm, s0l, N0l, blendingFactor, flipBCdir=False
    ):
    ...
    normProj = min(
        uvDir.dot(N0u[0, :3]), uvDir.dot(N0u[0, :3])
    )  # Even though the two normals should be the same, we still check both
    if normProj < 0:
        print("TFI WARNING:")
    ...
    dXdv[:, 0], dYdv[:, 0], dZdv[:, 0] = computeDerivativesOnLine(
        X[:, 0],
        Y[:, 0],
        Z[:, 0],
        X[0, 1],
    ...
    dXdu[-1, ::-1], dYdu[-1, ::-1], dZdu[-1, ::-1] = computeDerivativesOnLine(
        X[-1, ::-1],
        Y[-1, ::-1],
        Z[-1, ::-1],
        X[-2, -1],
    ...
    dXdv[:, -1], dYdv[:, -1], dZdv[:, -1] = computeDerivativesOnLine(
        X[:, -1],
        Y[:, -1],
        Z[:, -1],
        X[0, -2],
    ...
    dXdu[0, ::-1], dYdu[0, ::-1], dZdu[0, ::-1] = computeDerivativesOnLine(
        X[0, ::-1],
        Y[0, ::-1],
        Z[0, ::-1],
        X[1, -1],
