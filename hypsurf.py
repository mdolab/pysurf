...
class HypSurfMesh(object):
    ...
    def projection(self, rNext):
        ...
        # Correction step for bad normals (still need to understand what is happening)
        if np.isnan(np.max(NNext)):
            for ii in range(self.numNodes):
                if np.isnan(NNext[0,ii]):
                    print 'Fixing normal for:',rold[3*ii:3*ii+3]
                    ii_prev = ii
                    ii_next = ii
                    notFound = True
                    while notFound:
                        ii_prev= ii_prev - 1
                        if not np.isnan(NNext[0,ii_prev]):
                            notFound = False
                    notFound = True
                    while notFound:
                        ii_next= ii_next + 1
                        if not np.isnan(NNext[0,ii_next]):
                            notFound = False
                    NNext[:,ii] = 0.5*(NNext[:,ii_prev] + NNext[:,ii_next])
    ...
    def computeMatrices(self, r0, N0, S0, rm1, Sm1, layerIndex):
        ...
        def matrixBuilder(curr_index):
            ...
            if angle < 100*np.pi/180: # Corner detected
