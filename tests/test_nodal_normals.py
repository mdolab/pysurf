...
class TestNodalNormals(unittest.TestCase):
    ...
    def test_nodal_normals(self):
        ...
        nodal_normals_AD, nodal_normalsd_AD = adtAPI.adtapi.adtcomputenodalnormals_d(coor, coord, triaConn, quadsConn)
        ...
        nodal_normals = adtAPI.adtapi.adtcomputenodalnormals(coor, triaConn, quadsConn)

        # Compute directional derivative
        nodal_normalsd_FD = (nodal_normals - nodal_normals0) / stepSize

        # Compute dot product test
        dotProd = np.sum(coord * coorb_AD) - np.sum(nodal_normalsd_AD * nodal_normalsb)
        np.testing.assert_allclose(dotProd, 0, atol=3e-16)

        # Compare AD to FD
        np.testing.assert_allclose(nodal_normalsd_AD, nodal_normalsd_FD, atol=3e-8)
