...
# Load components
wing = pysurf.TSurfGeometry(
    "../../inputs/crm.cgns", ["w_upp", "w_low", "w_ted", "te_low_curve", "te_upp_curve", "w_le_curve"]
)
...
def generateWingBodyMesh(wingTranslation, wingRotation, meshIndex):
    ...
    Intersections[curveNames[1]] = Intersections[curveNames[1]].remesh(
        nNewNodes=150, spacing="hypTan", initialSpacing=0.1, finalSpacing=0.5
    )
    Intersections[curveNames[2]] = Intersections[curveNames[2]].remesh(
        nNewNodes=150, spacing="hypTan", initialSpacing=0.5, finalSpacing=0.1
    )
